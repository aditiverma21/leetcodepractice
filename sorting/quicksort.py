...
def quicksort1(array):
    ...
    #considering the middle element as pivot
    ...
    return quicksort1(low) + same + quicksort1(high)
        
'''

def quicksort(A,lb,ub):
    #print("Sort %d %d" %(lb, ub))
    if (ub>lb):
        loc=partion(A,lb,ub)
...
def partion(arr,lb,ub):
    #print("Partition %d %d" %(lb,ub)) 
    pivot=arr[lb]
    start=lb
    end=ub
    while end>start:
        #print(start,end,pivot)
        while ub>=start and pivot>=arr[start]:
            start+=1
        while end>=0 and arr[end]>pivot:
            end-=1
        #print(start,end)
        if end>=0 and ub>=start and end>start:
            temp=arr[end]
            arr[end]=arr[start]
            arr[start]=temp
